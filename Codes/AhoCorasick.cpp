// https://discuss.codechef.com/t/twostrs-editorial/64199
...
struct AhoCorasick {
  ...
  explicit AhoCorasick(const vector<pair<string, int>>& a)
      : currentNode(ROOT_ID), edges(2), suffixLink(2, FAKE_NODE_ID), sum(2, 0) {
  ...
  void setNode(int node) { currentNode = node; }

  void resetNode() { setNode(ROOT_ID); }

  long long getCurrentNodeSum() { return sum[currentNode]; }
